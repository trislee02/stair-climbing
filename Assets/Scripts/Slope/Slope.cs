...
public class Slope : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        float rise = Mathf.Tan(alpha/180*Mathf.PI) * segmentLength;
        float sinAlpha = Mathf.Sin(alpha / 180 * Mathf.PI);
        float cosAlpha = Mathf.Cos(alpha / 180 * Mathf.PI);

        Vector3 slopePosition = new Vector3(startPosition.x, startPosition.y + (sinAlpha * length/2) - slopeThickness - cosAlpha * slopeThickness, startPosition.z + (cosAlpha * length / 2) + rise);
        GameObject slopeObj;
        if (slopeSample != null)
        {
        ...
        slopeObj.transform.Rotate(-alpha, 0, 0);
        ...
        for (int i = 1; i <= length; i++)
        {
            if (i % wallStep == 0)
            {
                ...
                stairWall.transform.position = new Vector3(startPosition.x + 1.9f + (i == wallStep ? 1.5f : 0.05f), startPosition.y + wallHeight / 2 + sinAlpha*(i-1) - rise, startPosition.z + cosAlpha * (i + 1 - wallStep));
            ...
            if (handrailSample != null)
            {
                ...
                handrailInstance.transform.position = new Vector3(startPosition.x - width / 2f, startPosition.y + handrailHeight / 2 + sinAlpha * (i - 1) - rise, startPosition.z + cosAlpha * (i));
            ...
            if (i % enviromentSpawningStepDistance == 0 && envSampleLen > 0)
            {
                ...
                envObj.transform.position = new Vector3(startPosition.x - 6.5f - width / 2f, startPosition.y + (sinAlpha * (i - 1) - rise) - Random.Range(1f, 5f), startPosition.z + cosAlpha * (i));
